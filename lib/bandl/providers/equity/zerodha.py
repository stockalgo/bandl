...
from bandl.providers.equity.zerodha_account import ZerodhaAccountMixin
from bandl.providers.equity.zerodha_common import KITE_API
from bandl.providers.equity.zerodha_common import parse_kite_timestamp as _parse_kite_timestamp
...
def _kite_interval(iv: Interval | str) -> str:
    if isinstance(iv, Interval):
        if iv not in KITE_INTERVAL:
            raise ProviderError("zerodha", f"Unsupported interval for Kite: {iv}")
        return KITE_INTERVAL[iv]
    s = str(iv)
    if s in {"day", "minute", "3minute", "5minute", "15minute", "30minute", "60minute"}:
        return s
    raise ProviderError("zerodha", f"Unsupported interval string: {iv}")
...
class ZerodhaProvider(ZerodhaAccountMixin):
    ...
    def get_ohlcv(
        ...
        *,
        ...
        p_from = _ensure_utc(start).strftime("%Y-%m-%d %H:%M:%S")
        p_to = _ensure_utc(end).strftime("%Y-%m-%d %H:%M:%S")
