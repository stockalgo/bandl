"""CoinDCX USDT/INR futures (derivatives) account APIs.

Docs: https://docs.coindcx.com/ — Futures endpoints under
``/exchange/v1/derivatives/futures/``.

Note: futures authenticated requests use millisecond ``timestamp``;
spot ``/exchange/v1/orders/*`` uses seconds.
"""
...
_MAX_PAGE = 100  # API returns 422 above ~100 for futures list endpoints
...
def _parse_ms(raw: Any) -> datetime:
    ms = int(float(raw))
...
class CoinDCXFuturesMixin:
    """Futures account history for CoinDCXProvider."""
            ...
            for _ in range(50):
                ...
                for row in chunk:
                    ...
                    if start is not None and end is not None:
                        created = _parse_ms(row.get("created_at", 0))
                        if created < start or created >= end:
                            continue
                    pairs.add(str(row["pair"]))
                if len(chunk) < _MAX_PAGE:
                    break
                page += 1
        return pairs
    ...
    def _map_futures_fill(self: CoinDCXProvider, row: dict[str, Any]) -> AccountFill:
        ...
        executed = _parse_ms(ts)
        side = str(row.get("side", "buy")).lower()
        qty = Decimal(str(row["quantity"]))
        price = Decimal(str(row["price"]))
        pair = str(row.get("pair", ""))
        sym = _pair_to_canonical(pair)
        ...
        if granularity == PnLGranularity.SYMBOL:
            return [
                PnLRecord(
                    ...
                    total_pnl=pnl,
                    currency="USDT",
                    symbol=_pair_to_canonical(pair),
                    as_of=now,
