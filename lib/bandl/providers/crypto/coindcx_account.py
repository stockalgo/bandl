...
def _coindcx_ts_ms(dt: datetime) -> int:
    return int(dt.timestamp() * 1000)


def _parse_coindcx_ts(raw: Any) -> datetime:
    ms = int(raw)
...
class CoinDCXAccountMixin:
    ...
    def get_orders(self: CoinDCXProvider, filters: AccountFilters) -> list[AccountOrder]:
        ...
        for row in payload:
            ...
            created = _parse_coindcx_ts(row.get("created_at", row.get("timestamp", time.time())))
    ...
    def _get_spot_fills(self: CoinDCXProvider, filters: AccountFilters) -> list[AccountFill]:
        ...
        while safety < 200:
            ...
            body: dict[str, Any] = {
                ...
                "from_timestamp": _coindcx_ts_ms(filters.start),
                "to_timestamp": _coindcx_ts_ms(filters.end),
                "sort": "asc",
    ...
    def _map_coindcx_fill(self: CoinDCXProvider, row: dict[str, Any]) -> AccountFill:
        ...
        executed = _parse_coindcx_ts(row["timestamp"])
    ...
    def get_ledger_entries(self: CoinDCXProvider, filters: AccountFilters) -> list[LedgerEntry]:
        ...
        page = 1
        from bandl.providers.crypto.coindcx_futures import FUTURES_TRANSACTIONS, _parse_ms, _ts_ms
        ...
        while page <= 200:
            ...
            for row in chunk:
                if not isinstance(row, dict):
                    continue
                posted = _parse_ms(row.get("created_at", 0))
